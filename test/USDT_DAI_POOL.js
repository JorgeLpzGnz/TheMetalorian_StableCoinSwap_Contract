...
/**
 * Tests of the pool USDT - DAI 
 * token 1 = sC1 ( stable coin 1 ) = USDT
 * token 2 = sC2 ( stable coin 2 ) = DAI
 * 
...
describe("MetalorianSwap", function () {
	...
	async function deployMetalorianSwap() {
        ...
        // In the case of dai we deploy a copy with the same code for simplicity

		const sC2 = await ( await ethers.getContractFactory("Dai") ).deploy( 1 );
		...
		const metaSwap = await MetalorianSwap.deploy( sC1.address, sC2.address, "USDTDAI_LP", owner.address );
