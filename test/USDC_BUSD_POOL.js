...
/**
 * Tests of the pool USDC - BUSD 
 * token 1 = sC1 ( stable coin 1 ) = USDC
 * token 2 = sC2 ( stable coin 2 ) = BUSD
 * 
...
describe("Pool USDC - BUSD", function () {

	async function deployMetalorianSwap() {

		const [ owner, otherAccount ] = await ethers.getSigners()

		const sC1 = await ethers.getContractAt(tokensABI, addresses.usdc)

		const sC2 = await ethers.getContractAt(tokensABI, addresses.busd)
		...
		const metaSwap = await MetalorianSwap.deploy( sC1.address, sC2.address, "USDCBUSD_LP", owner.address );

		const amountToMint = parseEther("1000000000000000")

		await sC1.configureMinter( owner.address, amountToMint )

		// issue each token to the owner address

		await sC1.mint( owner.address, amountToMint )
		await sC2.increaseSupply( amountToMint )
