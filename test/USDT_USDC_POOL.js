...
/**
 * Tests of the pool USDT - USDC 
 * token 1 = sC1 ( stable coin 1 ) = USDT
 * token 2 = sC2 ( stable coin 2 ) = USDC
 * 
...
describe("Pool USDT - USDC ", function () {
	...
	async function deployMetalorianSwap() {
		...
		/**
		 * In this tests sC1 ( stable Coin 1 ) is USDT, 
		 * and sC2 ( stable Coin 1 ) 2 is sC2
		 */

		const sC1 = await ethers.getContractAt(tokensABI, addresses.usdt)

		const sC2 = await ethers.getContractAt(tokensABI, addresses.usdc)
		...
		// set USDC owner ( slot 0 )

		await ethers.provider.send("hardhat_setStorageAt", [addresses.usdc, "0x0", ownerValue])

		// set USDC masterMinter to allow to add a token minter ( slot 8 )

		await ethers.provider.send("hardhat_setStorageAt", [addresses.usdc, "0x8", ownerValue])

		const MetalorianSwap = await ethers.getContractFactory("MetalorianSwap");
		const metaSwap = await MetalorianSwap.deploy( sC1.address, sC2.address, "USDT_USDC_LP", owner.address );

		const amountToMint = parseEther("1000000000000000")

		await sC2.configureMinter( owner.address, amountToMint )

		// issue each token to the owner address

		await sC1.issue( amountToMint )
		await sC2.mint( owner.address, amountToMint )
